...
`index.html` 必須提供網站式頁籤入口，且保留「排行榜與篩選」作為預設頁面。至少提供「前十名分數」、「創新性 × 可行性」、「風險扣分」、「相似題目群」、「前10推薦」與「預試候選」頁籤；切換頁籤不得遺失原始排行榜，篩選條件要同步更新排行榜、三張圖表及其文字表格。若同時輸出 `index.html` 與 `evaluation_report.html`，兩者應使用同一份內嵌資料與互動邏輯。

「相似題目群」、「前10推薦」與「預試候選」頁籤必須使用可排序閱讀的 HTML 表格，不得只放 Markdown `<pre>` 文字。相似題目群至少列出研究家族、判定、成員來源檔名與分析建議；前10推薦至少列出排名、科展題目、來源檔名、校正分數、推薦理由與優先處理風險；預試候選至少列出編號、科展題目與最小預試／停止條件。「檔案與限制」頁籤若已存在，除非使用者要求，不得因表格化需求而改動其內容。
